...
def ensure_edge_driver(log_fn) -> str:
    ...
    if pinned:
        ...
        major = get_edge_major_version(log_fn)
        latest_url = f"https://msedgedriver.microsoft.com/LATEST_RELEASE_{major}_WINDOWS"
        ...
        if not ver:
            ...
            if m: ver = m.group(0)
        if not ver: raise RuntimeError("EdgeDriver Version Parsing Error")
    ...
    if not exe: raise RuntimeError("msedgedriver.exe Extraction Failed")
    exe.chmod(0o755)
    return str(exe)
